...
- pptx: 最初の見出しが表紙(タイトルスライド)になります。それ以降は「## スライドタイトル」ごとに新しいスライドになります。その下に本文(段落・箇条書き)を書いてください

箇条書き・番号付きリストは、項目の先頭に半角スペース2個を追加するごとに1階層深いサブ項目として扱われます(例:「- 親項目」の下に「  - 子項目」)。内容の構造に応じて、適切に階層化してください。

氏名・日付・金額などが複数件並ぶ一覧は、箇条書きではなくMarkdown表(| セル | セル |)で記述してください。表の方が見やすく整理されます。

[[FILE:...]]ブロックの前後に、通常の説明文を加えても構いません。1つの回答で複数のファイルを生成することも可能です。

内容を削って薄くする必要はありません。ただし、1つの見出し・1枚のスライドに詰め込みすぎず、内容量に応じて見出しやスライドを適切に分けてください(pptxは特に、1スライドに情報を詰め込みすぎないようにしてください)。
"""
...
def prepare_model_args(request_body, request_headers):
    ...
    if "gpt-5" in app_settings.azure_openai.model.lower():
    ...
    try:
        ...
        if "gpt-5" in app_settings.azure_openai.model.lower():
            response = await azure_openai_client.chat.completions.create(
                model=app_settings.azure_openai.model, messages=messages, max_completion_tokens=64
            )
        else:
            response = await azure_openai_client.chat.completions.create(
